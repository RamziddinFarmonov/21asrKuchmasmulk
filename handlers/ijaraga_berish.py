"""
Ijaralar - IJARAGA BERISH Bo'limi
Professional refactored version
Funksiyalar:
Foydalanuvchi mulkini ijaraga berish uchun ariza yuborish
...
MULK_TYPES = {
    "🏢 Ko'p qavatli uyni ijaraga berish": "Ko'p qavatli uy",
    "🏡 Hovli ijaraga berish": "Hovli",
    "🏬 Savdo do'konini ijaraga berish": "Savdo do'koni",
    "🏨 Mehmonxona ijaraga berish": "Mehmonxona",
    "🔄 Boshqa ijara joylarini berish": "Boshqa ijara joyi"
}
...
class RentGiveForm(StatesGroup):
    ...
    maydon = State()
    narx = State()
    muddat = State()  # Yangi: ijara muddati
    izoh = State()
    media = State()
    confirm = State()
...
def get_muddat_keyboard() -> ReplyKeyboardMarkup:
    """Ijara muddati tugmalari"""
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="1 oy"), KeyboardButton(text="3 oy")],
            [KeyboardButton(text="6 oy"), KeyboardButton(text="1 yil")],
            [KeyboardButton(text="Uzoq muddat"), KeyboardButton(text="Kelishiladi")],
            [KeyboardButton(text="🔙 Bekor qilish")]
        ],
        resize_keyboard=True
    )
...
def get_confirm_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ Tasdiqlash", callback_data="rent_give_confirm_yes"),
            InlineKeyboardButton(text="❌ Bekor qilish", callback_data="rent_give_confirm_no")
        ]
    ])
...
async def start_rent_give_form(message: Message, state: FSMContext):
    """Ijaraga berish formasini boshlash"""
    mulk_turi = MULK_TYPES[message.text]
    await state.clear()
    await state.update_data(mulk_turi=mulk_turi, media=[])
    await state.set_state(RentGiveForm.ism_familiya)

    await message.answer(
        f"🏠 <b>{mulk_turi} - Ijaraga berish</b>\n\n"
...
# ============================================================================
@router.message(RentGiveForm.ism_familiya)
async def process_name(message: Message, state: FSMContext):
    ...
    await state.set_state(RentGiveForm.viloyat)
...
# HANDLER'LAR - VILOYAT
# ============================================================================
@router.message(RentGiveForm.viloyat)
async def process_viloyat(message: Message, state: FSMContext):
    ...
    await state.set_state(RentGiveForm.joylashuv_detail)
...
# ============================================================================
@router.message(RentGiveForm.joylashuv_detail)
async def process_joylashuv(message: Message, state: FSMContext):
    ...
    await state.set_state(RentGiveForm.telefon)
...
# HANDLER'LAR - TELEFON
# ============================================================================
@router.message(RentGiveForm.telefon, F.contact)
async def process_phone_contact(message: Message, state: FSMContext):
    ...
    await state.set_state(RentGiveForm.maydon)
...
@router.message(RentGiveForm.telefon)
...
# HANDLER'LAR - MAYDON
# ============================================================================
@router.message(RentGiveForm.maydon)
async def process_maydon(message: Message, state: FSMContext):
    ...
    await state.set_state(RentGiveForm.narx)

    await message.answer(
        "💰 <b>Oylik ijara narxini kiriting:</b>\n\n"
        "Masalan:\n"
        "• <i>8 mln so'm/oy</i>\n"
        "• <i>$800/oy</i>\n"
        "• <i>Kelishiladi</i>",
...
# HANDLER'LAR - NARX
# ============================================================================
@router.message(RentGiveForm.narx)
async def process_narx(message: Message, state: FSMContext):
    ...
    await state.set_state(RentGiveForm.muddat)

    await message.answer(
        "📅 <b>Ijara muddatini tanlang:</b>",
        reply_markup=get_muddat_keyboard(),
        parse_mode="HTML"
    )
...
# HANDLER'LAR - MUDDAT
# ============================================================================
@router.message(RentGiveForm.muddat)
async def process_muddat(message: Message, state: FSMContext):
    """Ijara muddatini qabul qilish"""
    ...
    await state.set_state(RentGiveForm.izoh)
...
# HANDLER'LAR - IZOH
# ============================================================================
@router.message(RentGiveForm.izoh)
async def process_izoh(message: Message, state: FSMContext):
    ...
    await state.set_state(RentGiveForm.media)
...
# HANDLER'LAR - MEDIA
# ============================================================================
@router.message(RentGiveForm.media, F.photo)
...
@router.message(RentGiveForm.media, F.video)
...
@router.message(RentGiveForm.media, F.text)
async def finish_media(message: Message, state: FSMContext):
    ...
    await state.set_state(RentGiveForm.confirm)
    await message.answer(
        preview,
...
# ============================================================================
@router.callback_query(F.data == "rent_give_confirm_yes")
async def confirm_rent_give(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        # Ma'lumotlar bazasiga saqlash (type='rent' bilan!)
        obj_data = {
            'type': 'rent',  # ✅ MUHIM: Faqat ijara obyekti
            'mulk_turi': data['mulk_turi'],
            'joylashuv': data['joylashuv'],
            'narx': f"{data['narx']} (Muddat: {data['muddat']})",
            'izoh': data.get('izoh', ''),
            'media': media_str,
        ...
        logger.info(f"Ijaraga berish arizasi qabul qilindi: user_id={callback.from_user.id}, obj_id={obj_id}")
...
@router.callback_query(F.data == "rent_give_confirm_no")
async def cancel_rent_give(callback: CallbackQuery, state: FSMContext):
...
async def send_to_admin(bot: Bot, data: Dict, obj_id: int):
    ...
    try:
        ...
        izoh_text = data.get('izoh', '') or "Yo'q"
        text = (
            "🔔 <b>YANGI IJARAGA BERISH ARIZASI</b>\n"
            "━━━━━━━━━━━━━━━━━━━\n\n"
            f"🆔 <b>ID:</b> {obj_id}\n"
            f"🏠 <b>Mulk:</b> {data['mulk_turi']}\n"
            f"📍 <b>Joylashuv:</b> {data['joylashuv']}\n"
