"""
Ko'chmas Mulk - SOTIB OLISH Bo'limi
Professional refactored version
Funksiyalar:
Viloyat bo'yicha qidirish
Obyektlarni ko'rish (rasm/video bilan)
Ariza yuborish
...
MULK_TYPES = {
    "🏢 Ko'p qavatli uy sotib olish": "Ko'p qavatli uy",
    "🏡 Hovli sotib olish": "Hovli",
    "🏬 Savdo do'konini sotib olish": "Savdo do'koni",
    "🗺 Yer uchastkasini sotib olish": "Yer uchastkasi"
}
...
def format_object_preview(obj: Dict) -> str:
    ...
    joylashuv = obj['joylashuv'][:40] + '...' if len(obj['joylashuv']) > 40 else obj['joylashuv']
    return f"💰 {narx}\n📍 {joylashuv}"
...
def get_objects_keyboard(objects: List[Dict], page: int = 0) -> InlineKeyboardMarkup:
    ...
    for obj in page_objects:
        ...
        keyboard.append([InlineKeyboardButton(
            text=preview,
            callback_data=f"buy_view_{obj['id']}"
        )])
...
def get_object_detail_keyboard(obj_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            ...
            callback_data=f"buy_interest_{obj_id}"
        )],
        [InlineKeyboardButton(
            text="🔙 Orqaga",
            callback_data="buy_back_to_list"
        )]
    ])
...
async def process_viloyat(message: Message, state: FSMContext):
    ...
    # Obyektlarni qidirish ✅ TYPE='SALE' BILAN!
    try:
       
        objects = db.search_objects('sale', mulk_turi, viloyat)  # ✅ TO'G'RI CHAQIRUV
...
@router.callback_query(F.data == "buy_back_to_list")
...
# ============================================================================
@router.callback_query(F.data.startswith("buy_interest_"))
...
async def send_interest_to_admin(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        logger.info(f"Mijoz qiziqishi yuborildi: user_id={user_id}, obj_id={obj_id}")
...
# BOSHQA HANDLER'LAR
# ============================================================================
@router.callback_query(F.data == "back_to_kochmas")
async def back_to_kochmas_menu(callback: CallbackQuery, state: FSMContext):
    """Ko'chmas mulk menyusiga qaytish"""
    await state.clear()
    from handlers.common import kochmas_menu
    await callback.message.answer(
        "Ko'chmas mulk bo'limiga qaytdingiz:",
